...
# -*- coding: utf-8 -*-
"""
全A股批量采集脚本 - 从通达信本地目录采集所有股票数据
运行方式: python batch_collect_all.py [--limit N] [--forecast]
"""
...
def batch_collect(db_client, codes, fetch_forecast=True):
    """批量采集财务数据、公司概况、行情快照和盈利预测"""
    ...
    for i, code in enumerate(codes, 1):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"采集行情快照失败 {code}: {e}")
                stats['market_failed'] += 1
...
def main():
    parser = argparse.ArgumentParser(description='全A股批量采集脚本')
    ...
    if args.limit:
        ...
        logger.info(f"限制采集前 {args.limit} 只股票")
    ...
    try:
        ...
        logger.info(f"采集内容: 财务数据 + 公司概况 {'+ 盈利预测' if args.forecast else ''}")
        
        start_time = datetime.now()
        stats = batch_collect(db_client, codes, args.forecast)
